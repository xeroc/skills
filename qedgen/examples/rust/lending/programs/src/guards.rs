...
pub fn deposit<'info>(ctx: &mut Deposit<'info>, amount: u64) -> Result<(), ProgramError> {
    ...
    if ctx.pool.status != PoolStatus::Active as u8 {
        return Err(ProgramError::from(
            crate::errors::LendingError::InvalidLifecycle,
        ));
    }
    // R28 PDA check: ctx.pool matches its declared seeds
    {
        ...
        if quasar_lang::pda::verify_program_address(
            __seeds,
            &crate::ID,
            ctx.pool.to_account_view().address(),
        )
        .is_err()
        {
    ...
    if (*ctx.pool_vault.owner()) != (*ctx.pool.to_account_view().address()) {
...
pub fn repay<'info>(ctx: &mut Repay<'info>) -> Result<(), ProgramError> {
    // lifecycle: require status == Active
    if ctx.loan.status != LoanStatus::Active as u8 {
        return Err(ProgramError::from(
            crate::errors::LendingError::InvalidLifecycle,
        ));
    }
...
pub fn liquidate<'info>(ctx: &mut Liquidate<'info>) -> Result<(), ProgramError> {
    ...
    // R28 PDA check: ctx.loan matches its declared seeds
    {
        let __seeds: &[&[u8]] = &[
            b"loan",
            ctx.pool.to_account_view().address().as_ref(),
            ctx.loan.borrower.as_ref(),
            &[ctx.loan.bump],
        ];
        if quasar_lang::pda::verify_program_address(
            __seeds,
            &crate::ID,
            ctx.loan.to_account_view().address(),
        )
        .is_err()
        {
            return Err(ProgramError::from(LendingError::InvalidPda));
    ...
    if !(ctx.loan.amount.get() > ctx.loan.collateral.get()) {
        return Err(ProgramError::from(LendingError::AccountHealthy));
    }
